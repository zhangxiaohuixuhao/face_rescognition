...
class FusionNet(nn.Module):
    ...
    def forward(self, x):
        ...
        color_feas = self.color_moudle.forward_res3(ir)
        depth_feas = self.depth_moudle.forward_res3(ir)
        ir_feas = self.ir_moudle.forward_res3(ir)
        #[需填空]将3个模态 concat 融合,并实现MFE
        fea = torch.cat([color_feas, depth_feas, ir_feas], dim=1)
        index = np.random.randint(0, 3)
        if index == 0:
            temp_0 = torch.zeros(color_feas.shape)
            temp_1 = torch.ones(depth_feas.shape)
            temp_2 = torch.ones(ir_feas.shape)
        elif index == 1:
            temp_0 = torch.ones(color_feas.shape)
            temp_1 = torch.zeros(depth_feas.shape)
            temp_2 = torch.ones(ir_feas.shape)
        else:
            temp_0 = torch.ones(color_feas.shape)
            temp_1 = torch.ones(depth_feas.shape)
            temp_2 = torch.zeros(ir_feas.shape)
        temp = torch.cat([temp_0, temp_1, temp_2], dim=1)
        fea = torch.mul(fea, temp)
        x = self.res_4(fea)
        x = self.res_5(x)
