...
def run_test(config, dir):
    ...
    print('infer!')
    test_loss,out = do_valid_test(net, test_loader, criterion)
    print('%0.6f  %0.6f  %0.3f  (%0.3f) \n' % (test_loss[0], test_loss[1], test_loss[2], test_loss[3]))
    # out = infer_test(net, test_loader) #zh
    # print('done')

    # submission(out,save_dir+'_noTTA.txt', mode='test') #zh
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cycle_num', type=int, default=15)
    parser.add_argument('--cycle_inter', type=int, default=75)
